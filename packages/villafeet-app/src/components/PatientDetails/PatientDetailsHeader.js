...
const PatientDetailsHeader = (props) => {
  const {
    avatar, name, lastVisit, gender, children,
  } = props;
  const time = dateUtils.getFormatedDate(lastVisit);

  return (
    <section className="PatientDetailsHeader">
      <Link className="PatientDetailsHeader__back visible-xs" to="/pacientes" />
      ...
      <div>
        ...
        {time && (
        <h3 className="theme-body-small PatientDetailsHeader__consulta">
Última consulta:
